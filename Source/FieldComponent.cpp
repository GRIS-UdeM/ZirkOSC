...
//==============================================================================
FieldComponent::FieldComponent(ZirkOscjuceAudioProcessor* filter, SourceMover *mover)
:
	mFilter(filter),
	mMover(mover)
{
...
void FieldComponent::paint (Graphics& g)
{
	...
	for (int i = 0; i < mFilter->getNbrSources(); i++)
	{
		...
		float hue = (float)i / mFilter->getNbrSources() + 0.577251;
		if (hue > 1) hue -= 1;
...
void FieldComponent::mouseDown(const MouseEvent &event)
{
	...
	for (int i = mFilter->getNbrSources() - 1; i >= 0; i--)
	{
		FPoint p = getSourcePoint(i);
		float dx = ml.x - p.x;
		float dy = ml.y - p.y;
